...
plot.density.differences = function(
  ...
  # ### Check colors
  if (F %in% Rseb::is.color(colors)) {stop("The 'colors' parameter must be a single string or a 3-elements vector of strings including R-supported colors.")}
  if (length(colors) == 1) {colors = rep(colors, 3)}
  if (length(colors) == 2) {stop("The colors vector contains only two elements, three are required: Sample1, Sample2, No difference in this order.")}
  if (length(colors) > 3) {
    colors = colors[1:3]
  ...
  if (length(sample_names) < 2) {stop("At least two samples are required to compute comparisons.")}
