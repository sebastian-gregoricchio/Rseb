...
read.computeMatrix.file = function(matrix.file) {
  ...
  if (class(matrix.file) != "character" | length(matrix.file)  != 1 | !grepl(".gz$", matrix.file[1])) {
    stop("The matrix.file must be a single string indicating the full path to the matrix.file.gz generated by deepTools/computeMatrix function or by Rseb::computeMatrix.deeptools().")
  }
