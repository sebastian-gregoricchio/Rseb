...
#' @param colors Vector to define the colors of the lines. If only one value is provided or the number of values are less then the required ones only the first value will be used. All standard R.colors values are accepted. By default \code{c("blue", "red", "purple", "orange", "green")}.
    ...
    if (length(x) < n_samples &
      ...
      v = c(rep(x, n_samples))
                 ...
                 length(x) < n_samples) {
                   ...
                   message("# colors and/or line.type < # samples --> The first value have been used")
      } else (v = as.vector(x))
    return(v)
    }
